...
param(
$subscriptionId,
$resourceGroupNameMtm,
$existingWVDHostPoolNameMtm
)


#Obtain RdsRegistrationInfotoken

$Registered = Get-AzWvdRegistrationInfo -SubscriptionId "$subscriptionId" -ResourceGroupName "$resourceGroupNameMtm" -HostPoolName $existingWVDHostPoolNameMtm
...
{
    $Registered = New-AzWvdRegistrationInfo -SubscriptionId $subscriptionId -ResourceGroupName $resourceGroupNameMtm -HostPoolName $existingWVDHostPoolNameMtm -ExpirationTime (Get-Date).AddHours(4) -ErrorAction SilentlyContinue
}
