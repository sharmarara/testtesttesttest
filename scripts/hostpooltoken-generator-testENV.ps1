...
$Registered = Get-AzWvdRegistrationInfo -SubscriptionId "$subscriptionId" -ResourceGroupName "$resourceGroupName" -HostPoolName $existingWVDHostPoolName
...
{
    $Registered = New-AzWvdRegistrationInfo -SubscriptionId $subscriptionId -ResourceGroupName $resourceGroupName -HostPoolName $existingWVDHostPoolName -ExpirationTime (Get-Date).AddHours(4) -ErrorAction SilentlyContinue
}
